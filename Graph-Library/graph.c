...
void read_graph_from_file (struct graph *g, const char *filename)
{
    assert(g);
    ...
    for (int i = 0; i < E; i++)
    {
        if (!fscanf(file,"%d %d",&ids[0],&ids[1]))
        {
            fprintf(stderr,"Error reading input file!\n");
            exit(EXIT_FAILURE);
        }
        insert_edge_graph(g,ids[0],ids[1]);
        insert_edge_graph(g,ids[1],ids[0]);
    }

    fclose(file);
}

void generate_point (double pos[])
{
    for (int i = 0; i < 3; i++)
        pos[i] = rand() % 100;
}

void generate_edge (int ids[], const int V)
{
    for (int i = 0; i < 2; i++)
        ids[i] = rand() % V;
}

void generate_graph (struct graph *g, int V, int E)
{
    double pos[3];
    int ids[2];
